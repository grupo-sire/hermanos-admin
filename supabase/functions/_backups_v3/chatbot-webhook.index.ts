  ...
  try {
    ...
    if (agente) {
      ...
      const [
        ...
        supabase.from("produtos").select("nome, preco, descricao").eq("status", "active").limit(30),
        supabase.from("whatsapp_mensagens").select("mensagem, direcao, created_at").eq("telefone", telefone).eq("empresa_id", empresaId).order("created_at", { ascending: false }).limit(15),
      ]);
      ...
      if (webhookUrl) {
        try {
          ...
          console.error("Error delegating to AI agent n8n, falling back to direct Gemini:", aiErr.message);
        }
      }

      // Direct Gemini AI Agent fallback (when n8n is not set or fails)
      const GEMINI_API_KEY = Deno.env.get("GEMINI_API_KEY");
      if (GEMINI_API_KEY) {
        const historicoInvertido = (historico || []).reverse();
        const historicoTexto = historicoInvertido
          .map((h: any) => `${h.direcao === "recebida" ? "Cliente" : agente.nome || "Heloísa"}: ${h.mensagem}`)
          .join("\n");

        const servicosTxt = (servicos || []).map((s: any) => `• ${s.nome}: R$ ${Number(s.preco).toFixed(2)}${s.duracao_minutos ? ` (${s.duracao_minutos} min)` : ""}`).join("\n");
...
• TRATAMENTO FACIAL:
  - Máscara Black Limpeza de Pele Los Puttos 140ml (R$ 50,00) - Desintoxicação facial profunda com carvão ativado para remoção de cravos e oleosidade.`;
        ...
        const produtosTxt = (produtos && produtos.length > 0)
          ? `${produtos.map((p: any) => `• ${p.nome}: R$ ${Number(p.preco).toFixed(2)}${p.descricao ? ` - ${p.descricao}` : ""}`).join("\n")}\n\n(Catálogo Base Los Puttos):\n${produtosPadrao}`
          : produtosPadrao;

        const promptSistema = `VOCÊ É A ${agente.nome || "HELOÍSA"}, ATENDENTE VIRTUAL OFICIAL DA ${empresaData?.nome || "BARBEARIA HERMANOS"}.
TOM DE VOZ: ${agente.tom || "Simpática, acolhedora e profissional"}.
FALE COMO UMA PESSOA REAL: Natural, atenciosa e direta, sem saudações desnecessárias se a conversa já estiver em andamento.

🚨 REGRA DE OURO PARA AGENDAMENTOS (100% BOOKSY ONLINE):
1. VOCÊ NÃO MARCA HORÁRIOS PELO CHAT. Você não tem acesso a horários livres e NÃO pode agendar diretamente pelo WhatsApp.
2. NUNCA pergunte "Qual dia e horário você prefere?", "Para quando gostaria?" ou similar!
3. Se o cliente AINDA NÃO informou a unidade, pergunte gentilmente qual das 7 unidades ele prefere (Higienópolis, Osasco, Mooca, Tatuapé, Freguesia do Ó, São Caetano ou Itaim Bibi).
4. Se o cliente JÁ informou a unidade (ou assim que escolher), envie IMEDIATAMENTE o link do Booksy daquela unidade para ele escolher o dia, horário e o barbeiro de sua preferência!
...
⚠️ FORMATO OBRIGATÓRIO DE ENVIO DO LINK:
...
💈 PADRÃO DE CÓDIGOS DE BARBEIROS POR UNIDADE:
• A letra inicial do código do barbeiro indica a sua respectiva unidade:
  - M (ex: m5, M2) ➔ Mooca
  - H (ex: H1, H4) ➔ Higienópolis
  - O (ex: O2, O5) ➔ Osasco
  - T (ex: T1, T3) ➔ Tatuapé
  - F (ex: F2, F5) ➔ Freguesia do Ó
  - S (ex: S1, S3) ➔ São Caetano do Sul
  - I (ex: I2, I4) ➔ Itaim Bibi
• NUNCA diga que não utilizamos essa numeração nem que o número não existe! Reconheça a unidade com simpatia e envie o link oficial do Booksy daquela unidade para o cliente agendar com o profissional pelo app.

👑 ASSINATURA INFINITE E CANCELAMENTO:
• Nossos planos oficiais são:
  ...
  - Infinite Duos (R$ 199,89/mês): Cortes + Barbas ilimitados + Sobrancelha inclusa. (Nosso link oficial: https://barbeariahermanos.com.br/checkout?plan=infinite-duos)
  - Infinite Barb (R$ 129,89/mês): Barbas ilimitadas com toalha quente. (Nosso link oficial: https://barbeariahermanos.com.br/checkout?plan=infinite-barb)
  - Infinite Plus (R$ 34,90/mês): Pacote com hidratação, sobrancelha, limpeza de pele e depilação.
• ⚠️ REGRA DO PLANO INFINITE PLUS (R$ 34,90/mês): O plano Infinite Plus é um pacote adicional (add-on). Você pode apresentar e explicar o plano normalmente a qualquer cliente, mas DEVE SEMPRE ESCLARECER com clareza e gentileza que a assinatura do Infinite Plus só pode ser concluída se o cliente já possuir outro plano ativo (Cuts, Duos ou Barb). (Nosso link oficial: https://barbeariahermanos.com.br/checkout?plan=infinite-plus)
• 🚫 NUNCA utilize a palavra "checkout". Use SEMPRE a expressão "nosso link oficial".

✂️ REGRAS DE CANCELAMENTO:
• ✂️ CANCELAMENTO DE AGENDAMENTO OU HORÁRIO: Se o cliente pedir para cancelar ou alterar um agendamento de horário (corte ou barba), explique com cortesia que para cancelar ou alterar o horário ele deve acessar diretamente o aplicativo do Booksy (ou o próprio link oficial de agendamento online da unidade dele).
• 🚪 CANCELAMENTO DE ASSINATURA INFINITE: O cancelamento da Assinatura Infinite (mensalidade) é realizado EXCLUSIVAMENTE de forma presencial na recepção da unidade onde o cliente contratou a assinatura.

📦 PRODUTOS EXCLUSIVOS HERMANOS (LINHA LOS PUTTOS):
A Barbearia Hermanos possui sua linha própria exclusiva de cosméticos masculinos: **LOS PUTTOS**.
Todos os produtos são profissionais e estão disponíveis para pronta-entrega na recepção de TODAS as nossas 7 unidades!
${produtosTxt}

Diretrizes sobre produtos:
- Se o cliente perguntar sobre produtos ou cosméticos, apresente os itens com simpatia e informe os valores.
- Todos os produtos podem ser adquiridos presencialmente na recepção de qualquer uma de nossas unidades.
- Recomende o produto ideal para a necessidade do cliente (pomadas para fixação, balm/óleo para barba, máscaras faciais).

💈 SERVIÇOS:
${servicosTxt}

${agente.instrucoes_extras ? `[INSTRUÇÕES EXTRAS DO CRM]:\n${agente.instrucoes_extras}` : ""}
...
RESPONDA AGORA COMO ${agente.nome || "HELOÍSA"} MANTENDO TOTAL CONTINUIDADE AO DIÁLOGO:`;

        const modelosGemini = ["gemini-3.8-flash", "gemini-3.6-flash", "gemini-3.7-flash"];
          ...
          try {
            ...
            for (let t = 1; t <= 3; t++) {
              ...
              if (gRes.ok) {
                ...
                let replyText = gData?.candidates?.[0]?.content?.parts?.map((p: any) => p.text).join("\n") || "";
                if (replyText && replyText.trim()) {
                  let limpo = replyText.trim();
                  ...
                  await sendWhatsApp(supabase, telefone, limpo, instanceName, empresaId);
                  return ok({ status: "ai_agent_direct_gemini_responded" });
                }
                break;
              } else if ((gRes.status === 429 || gRes.status === 503) && t < 3) {
            ...
            console.error(`Gemini model ${modelName} error:`, gErr.message);
          }
        }
      }
    }
    ...
    if (bookingState) {
      if (msgLower.includes("cancel") || msgLower === "0") {
